...
function autoComplete($table,$selection,$name, $flag, $user_id, $j)
{
    ...
    if($nrows > 15)
    {
		$sel = dbHelp::fetchRowByIndex($res);
		$str = $sel[0];
...
function fillCombos ($table,$selection,$name,$extra='',$extraecho='', $user_id,$j)
{
    ...
    //echo $sql;
    for ($k=0;$k< dbHelp::numberOfRows($res);$k++) {
    ...
    if($nres != 0){ //user has restriction for this table
	    while($row = dbHelp::fetchRowByIndex($res)){
		    if($table == $row[2]){
			    ...
			    $res2 = dbHelp::query($sql) or die($sql); //mysql_error().$sql);
			    while ($fkrow = dbHelp::fetchRowByIndex($res2)){
				    if($table == $fkrow[0]){
					    $row[1] = str_replace(",","','",$row[1]);
